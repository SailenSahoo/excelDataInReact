  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <button onClick={() => setRegion("NAM")} style={{ marginRight: "10px", padding: "5px 10px", backgroundColor: region === "NAM" ? "#007BFF" : "#ccc", color: "white", border: "none", cursor: "pointer" }}>NAM</button>
        <button onClick={() => setRegion("APAC")} style={{ padding: "5px 10px", backgroundColor: region === "APAC" ? "#007BFF" : "#ccc", color: "white", border: "none", cursor: "pointer" }}>APAC</button>
      </div>
        ...
        <tbody>
          {Object.entries(groupedByTemplate).map(([template, projects]) => (
            <React.Fragment key={template}>
              <tr style={{ cursor: "pointer", backgroundColor: "#e6f2ff" }} onClick={() => setExpandedTemplate(expandedTemplate === template ? null : template)}>
